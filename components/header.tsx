...
export default function Header() {
    ...
    return (
        <header className="flex w-full h-[4.5rem] bg-transparent">
            ...
            <nav className="md:flex hidden justify-end gap-4 right-0 w-full items-center text-white list-none">
                {links.map((link) => (
                    <li key={link.hash}>
                        <Link
            ...
            <nav
                className={
                    navBtn
                        ? "md:hidden text-center items-center text-white list-none absolute w-full top-[4.6rem] flex-col bg-black bg-opacity-70 backdrop-blur-[5px] rounded-b-lg"
                        : "hidden"
                }
            >
            ...
            <div className={"flex md:hidden self-center ml-auto mr-[3rem] text-2xl text-white hover:cursor-pointer"}>
